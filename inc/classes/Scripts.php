...
trait Scripts {
  public static function enqueue_scripts() {
    ...
    // $family を上書きするHook
		$family = apply_filters('takasuohana_theme_font_family', $family);
    ...
    // style.css
    wp_enqueue_style('style', TAKASUOHANA_THEME_URI . '/style.css', [], $version);
    // init.js
    wp_enqueue_script('init', TAKASUOHANA_THEME_URI . '/init.js', [], $version, true);
    ...
    if (is_child_theme()) {
      ...
      // style.css
      wp_enqueue_style('custom', TAKASUOHANA_CHILD_URI . '/style.css', [], $version);
      // init.js
      if (file_exists(TAKASUOHANA_CHILD_DIR . '/init.js')) {
        wp_enqueue_script('custom', TAKASUOHANA_CHILD_URI . '/init.js', [], $version, true);
      }
    }
  }
}
